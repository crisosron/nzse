  ...
  return (
    <div className="Dropdown w-100 min-h-20 pl-2 py-2 transition-all duration-150 ease-linear">
      {items && items.map((item, index) =>
        <SidebarLinkBlock key={`sidebar-dropdown-block-${title}-item-${index}`} title={item.title} page={item} className="text-sm mb-2 last:mb-0" />
      )}
    </div>
  )
}
...
const SidebarDropdownBlock = ({ sidebarDropdownTitle: title, pages: pagesData }) => {
  ...
  // TODO: Currently, links to pages that do not match the audience are allowed
